pipeline {
    ...
    stages {
        ...
        stage('Security Scan') {
            steps {
                script {
                    def trivyOutput = sh(script: "trivy image rudravasu2021/${DOCKER_IMAGE_NAME}:${BUILD_NUMBER}  --exit-code 1 --severity HIGH", returnStdout: true).trim()
